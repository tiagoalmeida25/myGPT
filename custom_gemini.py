...
#  request={"prompt": prompt, "image_size": image_size, "num_images": num_images}
# )
...
class State:
    ...
    def start_chat(self):
        self.chat = genai.GenerativeModel(
            ...
            generation_config=genai.GenerationConfig(temperature=0.8, top_p=1, top_k=1),
        )
...
class CustomAssistentState(State):
    def __init__(self):
        super().__init__()
...
if __name__ == "__main__":
    state = CustomAssistentState()
    section = CustomAssistentSection(state)
    section.run()
